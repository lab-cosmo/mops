...
torch::Tensor mops_torch::sparse_accumulation_scatter_add_with_weights(
    torch::Tensor A, torch::Tensor B, torch::Tensor C, torch::Tensor W,
    torch::Tensor indices_A, torch::Tensor indices_W_1,
    torch::Tensor indices_W_2, torch::Tensor indices_output_1,
    torch::Tensor indices_output_2, int64_t output_size) {
    return SparseAccumulationScatterAddWithWeights::apply(
        A, B, C, W, indices_A, indices_W_1, indices_W_2, indices_output_1,
        indices_output_2, output_size);
}
...
torch::Tensor SparseAccumulationScatterAddWithWeights::forward(
    ...
    int64_t output_size) {
    details::check_all_same_device({A, B, C, W, indices_A, indices_W_1, indices_W_2,
                           indices_output_1, indices_output_2});
    details::check_floating_dtype({A, B, C, W});
    details::check_integer_dtype({indices_A, indices_W_1, indices_W_2, indices_output_1,
                         indices_output_2});
    details::check_number_of_dimensions(A, 2, "A",
                               "sparse_accumulation_scatter_add_with_weights");
    details::check_number_of_dimensions(B, 2, "B",
                               "sparse_accumulation_scatter_add_with_weights");
    details::check_number_of_dimensions(W, 3, "W",
                               "sparse_accumulation_scatter_add_with_weights");
    details::check_number_of_dimensions(indices_A, 1, "indices_A",
                               "sparse_accumulation_scatter_add_with_weights");
    details::check_number_of_dimensions(indices_W_1, 1, "indices_W_1",
                               "sparse_accumulation_scatter_add_with_weights");
    details::check_number_of_dimensions(indices_W_2, 1, "indices_W_2",
                               "sparse_accumulation_scatter_add_with_weights");
    details::check_number_of_dimensions(indices_output_1, 1, "indices_output_1",
                               "sparse_accumulation_scatter_add_with_weights");
    details::check_number_of_dimensions(indices_output_2, 1, "indices_output_2",
                               "sparse_accumulation_scatter_add_with_weights");
    ...
    if (A.device().is_cpu()) {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            A.scalar_type(), "sparse_accumulation_scatter_add_with_weights",
            [&]() {
        ...
        C10_THROW_ERROR(ValueError, "sparse_accumulation_scatter_add_with_"
                                    "weights is not implemented for device " +
                                        A.device().str());
    }

    if (A.requires_grad() || B.requires_grad() || W.requires_grad()) {
        ctx->save_for_backward({A, B, C, W, indices_A, indices_W_1, indices_W_2,
                                indices_output_1, indices_output_2});
    }
...
std::vector<torch::Tensor> SparseAccumulationScatterAddWithWeights::backward(
    ...
    if (A.device().is_cpu()) {
        AT_DISPATCH_FLOATING_TYPES(
            A.scalar_type(), "sparse_accumulation_scatter_add_with_weights_vjp",
            [&]() {
                ...
                mops::sparse_accumulation_scatter_add_with_weights_vjp<
                    scalar_t>(mops_grad_A, mops_grad_B, mops_grad_W,
    ...
    return {grad_A,          grad_B,          torch::Tensor(),
