...
torch::Tensor SparseAccumulationScatterAddWithWeights::forward(
    ...
    torch::Tensor output;
    if (A.device().is_cpu()) {
        output = torch::zeros({W.size(0), output_size, W.size(2)},
            torch::TensorOptions().dtype(A.scalar_type()).device(A.device())
        );
        assert(output.is_contiguous());

        AT_DISPATCH_FLOATING_TYPES(A.scalar_type(), "sparse_accumulation_scatter_add_with_weights", [&](){
        ...
        C10_THROW_ERROR(ValueError,
            "sparse_accumulation_scatter_add_with_weights is not implemented for device " + A.device().str()
        );
    }

    if (A.requires_grad() || B.requires_grad() || W.requires_grad()) {
        ctx->save_for_backward({A, B, C, W, indices_A, indices_W_1, indices_W_2, indices_output_1, indices_output_2});
    }
...
std::vector<torch::Tensor> SparseAccumulationScatterAddWithWeights::backward(
    ...
    if (A.device().is_cpu()) {
        AT_DISPATCH_FLOATING_TYPES(A.scalar_type(), "sparse_accumulation_scatter_add_with_weights_vjp", [&](){
    ...
    return {grad_A, grad_B, torch::Tensor(), grad_W, torch::Tensor(), torch::Tensor(), torch::Tensor(), torch::Tensor(), torch::Tensor()};
}
