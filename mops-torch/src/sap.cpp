...
torch::Tensor mops_torch::sparse_accumulation_of_products(
    ...
    torch::Tensor indices_B, torch::Tensor indices_output,
    int64_t output_size) {
    return SparseAccumulationOfProducts::apply(A, B, C, indices_A, indices_B,
...
torch::Tensor SparseAccumulationOfProducts::forward(
    ...
    details::check_number_of_dimensions(indices_B, 1, "indices_B",
                               "sparse_accumulation_of_products");
    ...
    if (A.device().is_cpu()) {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            A.scalar_type(), "sparse_accumulation_of_products", [&]() {
        ...
        C10_THROW_ERROR(
            ValueError,
            "sparse_accumulation_of_products is not implemented for device " +
                A.device().str());
    }
