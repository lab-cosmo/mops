    ...
    for (const auto& tensor : tensors) {
        if (tensor.device() != device) {
            C10_THROW_ERROR(ValueError,
                            "All tensors must be on the same device, found" +
                                tensor.device().str() + " and " + device.str());
        }
    }
}
...
void mops_torch::details::check_number_of_dimensions(torch::Tensor tensor, int64_t expected,
