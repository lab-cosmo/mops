...
torch::Tensor
mops_torch::homogeneous_polynomial_evaluation(torch::Tensor A, torch::Tensor C,
                                              torch::Tensor indices_A) {
    return HomogeneousPolynomialEvaluation::apply(A, C, indices_A);
}

torch::Tensor
HomogeneousPolynomialEvaluation::forward(torch::autograd::AutogradContext *ctx,
    ...
    details::check_number_of_dimensions(indices_A, 2, "indices_A",
    ...
    if (A.device().is_cpu()) {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            A.scalar_type(), "homogeneous_polynomial_evaluation", [&]() {
        ...
        C10_THROW_ERROR(
            ValueError,
            "homogeneous_polynomial_evaluation is not implemented for device " +
                A.device().str());
    }
        ...
        AT_DISPATCH_FLOATING_TYPES(
            A.scalar_type(), "homogeneous_polynomial_evaluation_vjp", [&]() {
                ...
                mops::homogeneous_polynomial_evaluation_vjp<scalar_t>(
                    mops_grad_A, details::torch_to_mops_1d<scalar_t>(grad_output),
                    details::torch_to_mops_2d<scalar_t>(A),
                    details::torch_to_mops_1d<scalar_t>(C),
                    details::torch_to_mops_2d<int32_t>(indices_A));
            });
