...
from mops.reference_implementations import (
    sparse_accumulation_of_products_vjp as ref_sap_vjp,
)

import mops
from mops import sparse_accumulation_of_products_vjp as sap_vjp
...
def valid_arguments():
    ...
    return grad_output, A, B, C, indices_A, indices_B, indices_output


def test_sap_vjp(valid_arguments):
    grad_output, A, B, C, indices_A, indices_B, indices_output = valid_arguments

    ref_grad_A, ref_grad_B = ref_sap_vjp(
        grad_output, A, B, C, indices_A, indices_B, indices_output
    )
    grad_A, grad_B = sap_vjp(grad_output, A, B, C, indices_A, indices_B, indices_output)
    assert np.allclose(ref_grad_A, grad_A)
    assert np.allclose(ref_grad_B, grad_B)


def test_sap_vjp_wrong_type(valid_arguments):
    grad_output, A, B, C, indices_A, indices_B, indices_output = valid_arguments
    A = A.astype(np.int32)
...
def test_sap_vjp_wrong_number_of_dimensions(valid_arguments):
    grad_output, A, B, C, indices_A, indices_B, indices_output = valid_arguments
    A = A[..., np.newaxis]
...
def test_sap_vjp_size_mismatch(valid_arguments):
    grad_output, A, B, C, indices_A, indices_B, indices_output = valid_arguments
    C = C[:5]
...
def test_sap_vjp_out_of_bounds(valid_arguments):
    grad_output, A, B, C, indices_A, indices_B, indices_output = valid_arguments
...
def test_sap_vjp_cupy(valid_arguments):
    grad_output, A, B, C, indices_A, indices_B, indices_output = valid_arguments
